...
class VGGBase(nn.Module):
    ...
    def forward(self, image):
        ...
        out = self.pool5(
            out)  # (N, 512, 19, 19), pool5 does not reduce dimensions
...
class PredictionConvolutions(nn.Module):
    ...
    def __init__(self, n_classes):
        ...
        self.loc_conv4_3 = nn.Conv2d(512,
                                     n_boxes['conv4_3'] * 4,
                                     kernel_size=3,
                                     padding=1)
        self.loc_conv7 = nn.Conv2d(1024,
                                   n_boxes['conv7'] * 4,
                                   kernel_size=3,
                                   padding=1)
        self.loc_conv8_2 = nn.Conv2d(512,
                                     n_boxes['conv8_2'] * 4,
                                     kernel_size=3,
                                     padding=1)
        self.loc_conv9_2 = nn.Conv2d(256,
                                     n_boxes['conv9_2'] * 4,
                                     kernel_size=3,
                                     padding=1)
        self.loc_conv10_2 = nn.Conv2d(256,
                                      n_boxes['conv10_2'] * 4,
                                      kernel_size=3,
                                      padding=1)
        self.loc_conv11_2 = nn.Conv2d(256,
                                      n_boxes['conv11_2'] * 4,
                                      kernel_size=3,
                                      padding=1)

        # Class prediction convolutions (predict classes in localization boxes)
        self.cl_conv4_3 = nn.Conv2d(512,
                                    n_boxes['conv4_3'] * n_classes,
                                    kernel_size=3,
                                    padding=1)
        self.cl_conv7 = nn.Conv2d(1024,
                                  n_boxes['conv7'] * n_classes,
                                  kernel_size=3,
                                  padding=1)
        self.cl_conv8_2 = nn.Conv2d(512,
                                    n_boxes['conv8_2'] * n_classes,
                                    kernel_size=3,
                                    padding=1)
        self.cl_conv9_2 = nn.Conv2d(256,
                                    n_boxes['conv9_2'] * n_classes,
                                    kernel_size=3,
                                    padding=1)
        self.cl_conv10_2 = nn.Conv2d(256,
                                     n_boxes['conv10_2'] * n_classes,
                                     kernel_size=3,
                                     padding=1)
        self.cl_conv11_2 = nn.Conv2d(256,
                                     n_boxes['conv11_2'] * n_classes,
                                     kernel_size=3,
                                     padding=1)
    ...
    def forward(self, conv4_3_feats, conv7_feats, conv8_2_feats, conv9_2_feats,
        ...
        l_conv8_2 = l_conv8_2.permute(0, 2, 3,
        ...
        l_conv10_2 = l_conv10_2.permute(0, 2, 3,
                                        1).contiguous()  # (N, 3, 3, 16)
        l_conv10_2 = l_conv10_2.view(batch_size, -1, 4)  # (N, 36, 4)

        l_conv11_2 = self.loc_conv11_2(conv11_2_feats)  # (N, 16, 1, 1)
        l_conv11_2 = l_conv11_2.permute(0, 2, 3,
                                        1).contiguous()  # (N, 1, 1, 16)
        l_conv11_2 = l_conv11_2.view(batch_size, -1, 4)  # (N, 4, 4)

        # Predict classes in localization boxes
        c_conv4_3 = self.cl_conv4_3(
            conv4_3_feats)  # (N, 4 * n_classes, 38, 38)
...
class SSD300(nn.Module):
    ...
    def __init__(self, model_base, n_classes):
        ...
        self.rescale_factors = nn.Parameter(torch.FloatTensor(
            1, 512, 1, 1))  # there are 512 channels in conv4_3_feats
    ...
    def forward(self, image):
        ...
        norm = conv4_3_feats.pow(2).sum(dim=1,
                                        keepdim=True).sqrt()  # (N, 1, 38, 38)
        ...
        locs, classes_scores = self.pred_convs(
            conv4_3_feats, conv7_feats, conv8_2_feats, conv9_2_feats,
            conv10_2_feats,
        ...
        for i in range(batch_size):
            ...
            for c in range(1, self.n_classes):
                ...
                suppress = torch.zeros(
                    (n_above_min_score),
                    dtype=torch.bool).cuda()  # (n_qualified)
                ...
                image_labels.append(
                    torch.LongTensor((~suppress).sum().item() * [c]).cuda())
            ...
            if len(image_boxes) == 0:
                image_boxes.append(
                    torch.FloatTensor([[0., 0., 1., 1.]]).cuda())
...
class MultiBoxLoss(nn.Module):
    ...
    def __init__(self, priors_cxcy, threshold=0.5, neg_pos_ratio=3, alpha=1.):
        ...
        self.priors_cxcy = priors_cxcy.cuda()
        self.priors_xy = cxcy_to_xy(priors_cxcy).cuda()
    ...
    def forward(self, predicted_locs, predicted_scores, boxes, labels):
        ...
        true_locs = torch.zeros((batch_size, n_priors, 4),
                                dtype=torch.float,
                                device=tensor_device)  # (N, 8732, 4)
        true_classes = torch.zeros((batch_size, n_priors),
                                   dtype=torch.long,
                                   device=tensor_device)  # (N, 8732)
        ...
        for i in range(batch_size):
            ...
            object_for_each_prior[prior_for_each_object] = torch.LongTensor(
                range(n_objects)).to(tensor_device)
